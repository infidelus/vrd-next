...
class LoggingPage(SettingsPage):
    TITLE = QT_TRANSLATE_NOOP("Settings", "Logging")
    ...
    def build(self):
        ...
        keep_row.addWidget(QLabel(self.tr("Number of log files to keep:")))
        ...
        self._log_max_files.setToolTip(
            self.tr("The oldest of the editor's log files beyond this many are deleted "
        ...
        self.add(hint(
            self.tr("Where the editor writes its per-day log files, and how many it "
            "keeps. The Watcher is a separate application and keeps its own "
            "logs - set those in the Watcher's own settings window.")
        ))

        self._verbose_logging = QCheckBox(self.tr("Verbose logging"))
